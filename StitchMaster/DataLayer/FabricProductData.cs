    ...
    public class FabricProductData
    {
        ...
        public List<FabricProduct> GetAllObjects()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                FabricProduct product = new FabricProduct(int.Parse(dr["fabric_id"].ToString()), dr["title"].ToString(), dr["description"].ToString(), CuurentColor, dr["material"].ToString(), dr["gender"].ToString(), int.Parse(dr["price_per_meter"].ToString()), int.Parse(dr["in_stock_qty"].ToString()), 0, dr["image_url"].ToString());
                allProducts.Add(product);
            }
            return allProducts;
        }
        public void UpdateObject(FabricProduct Product)
        {
            string dml = $"Update fabric_product Set `title` = '{Product.FabricProductTitle}',`description`='{Product.FabricProductDescription}',`color_id` = '{Product.FabricColor.ColorID}',`material` = '{Product.FabricMaterial}', `gender` = '{Product.Gender[0]}',`price_per_meter` = '{Product.PricePerMeter}',`in_stock_qty` = '{Product.StockQuantity}',image_url = '{Product.ImageURl}' where fabric_id = {Product.FabricProductID};";
            DatabaseHelper.Instance.ExecuteQuery(dml);
        }
        public void DeleteObject(FabricProduct Product)
        {
